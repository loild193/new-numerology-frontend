...
import { Input } from '@models/api/admin/deleteUser'
import { deleteUser } from '@utils/api/admin/deleteUser'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'DELETE') {
    try {
      ...
      const { success, response, error, message } = await deleteUser({
        ...requestBody,
    ...
    res.status(400).json({ success: false, response: null, error: true, message: 'Invalid method. DELETE only.' })
  }
}
