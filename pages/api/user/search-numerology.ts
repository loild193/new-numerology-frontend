...
import { Input } from '@models/api/user/search-numerology'
import { searchNumerology } from '@utils/api/user/search-numerology'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'POST') {
    try {
      ...
      const { success, response, error, message } = await searchNumerology({
        ...requestBody,
