...
import {
  ...
  ERROR_MAPPING,
  ServerResponse as UpdateSearchAmountLeftServerResponse,
} from '@models/api/admin/updateSearchAmountLeft'
...
const updateSearchAmountLeft = async (input: { id: string; searchAmountLeft: number }) => {
  try {
    const response = await fetch('/api/admin/update-search-amount-left', {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    const rawResponse = (await response.json()) as UpdateSearchAmountLeftServerResponse
...
export const UserUpdateSearchNumerology = () => {
  ...
  const { mutate, isLoading: isUpdating } = useMutation({
    mutationFn: updateSearchAmountLeft,
    onSuccess: (data) => {
  ...
  const onUpdateSearchAmountLeft = () => {
    mutate({ id, searchAmountLeft })
  }

  const { username, email, phone, userId, searchAmountLeft } = userForm
  const isButtonDisabled = isUpdating || !searchAmountLeft
  ...
  return (
    <div className="pb-8">
      ...
      <div className="mt-6 mb-8 flex flex-col gap-y-4">
        ...
        <Input label="Tên đăng nhập" name="userId" disabled value={userId ?? ''} onChange={onChangeInput} />
        <Input
      ...
      <div className="flex justify-center">
        <div className="min-w-[160px]">
          <Button
            ...
            onClick={onUpdateSearchAmountLeft}
          />
        </div>
      </div>
    </div>
